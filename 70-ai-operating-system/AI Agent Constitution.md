...
17. **Treat the approved default branch `main` as the single canonical integration baseline. Parallel branches may diverge during authorised work, but before any PR is declared merge-ready or Founder merge approval is requested, the PR head must include the latest `main` as an ancestor. If `main` advances before merge, refresh the PR from the new `main`, resolve any conflict deliberately, rerun required assurance on the new exact head and obtain renewed Founder merge approval for that refreshed head.**
...
10. **For governed branch work, create the branch from the then-current `main`. Do not continuously chase `main` during ordinary parallel development; instead perform a mandatory integration-baseline refresh when the PR is otherwise ready to enter its final merge gate.**
