...
export function validateFoundationInternalLearningProofSource(source) {
  return {
...
export function parseFoundationInternalLearningProofIssueComment(body, expectedMarker) {
  ...
  return validateFoundationInternalLearningProofSource(source)
}

export function resolveFoundationInternalLearningProofSource({ eventName, issueCommentBody, expectedMarker, workflowInputs = {} }) {
  if (eventName === 'issue_comment') return parseFoundationInternalLearningProofIssueComment(issueCommentBody, expectedMarker)
  if (eventName === 'workflow_dispatch') return validateFoundationInternalLearningProofSource(workflowInputs)
  throw new Error(`Unsupported internal learning proof trigger event: ${eventName || 'unknown'}`)
}

export function appendFoundationInternalLearningProofSourceEnv(source, githubEnvPath) {
  requireValue(githubEnvPath, 'GITHUB_ENV')
  const validated = validateFoundationInternalLearningProofSource(source)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    const source = resolveFoundationInternalLearningProofSource({
    ...
    appendFoundationInternalLearningProofSourceEnv(source, process.env.GITHUB_ENV)
