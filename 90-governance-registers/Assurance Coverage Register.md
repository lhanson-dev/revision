...
The production incident in which the planner frontend was deployed before its Supabase schema existed exposed a separate path-to-live gap. The `planner-v1` readiness contract is live, required protected Edge Functions are deployed, and PR #66 established prospective Founder-approval lineage correlation. PTL-03 remains Partial because the post-merge push-triggered Pages deployment/smoke for the first marker-governed merge has not yet been independently observed in the current evidence source; it is not backfilled or assumed.

`revision_release_readiness()` remains least-privilege `SECURITY INVOKER`. SEC-03 is now Covered because the remaining repository-level secret/config scanning requirement is enforced before downstream CI, while public publishable browser configuration remains correctly treated as public. Supabase leaked-password protection is an external Auth setting and is not implied by SEC-03.
