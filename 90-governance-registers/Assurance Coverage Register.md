...
| SEC-06 | Level 3/4 PRs receive independent static security analysis; dependency-changing PRs additionally receive fail-closed dependency vulnerability analysis | High | CodeQL + change-scoped dependency vulnerability audit | `Revision CI` `Independent security and dependency analysis` job; assurance plan | Covered | CodeQL is mandatory for Level 3/4; dependency audit is mandatory when governed manifests/lockfiles change; external registry failure must not suppress unrelated CodeQL |
