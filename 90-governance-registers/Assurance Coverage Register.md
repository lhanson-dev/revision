...
| DATA-04 | Planner assessments, availability, bounded preferences and activity events remain learner-owned and planning context does not become mastery evidence | Critical | Database/RLS + domain integration | `supabase/tests/adaptive-planner-verification.sql`; planner engine/service separation | Partial | Execute planner SQL/RLS verification automatically and add a database-backed ownership/reload integration journey |
...
| SEC-04 | Planner assurance aggregates re-authorise server-side and do not expose cross-user planner rows to browser roles | Critical | SQL/integration + protected server boundary | `planner-operations` implementation and planner metrics migration | Partial | Add repeatable 401/403/admin-success integration evidence and production readiness verification |
