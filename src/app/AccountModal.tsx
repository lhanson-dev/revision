...
export function AccountModal({
  ...
  return (
    <>
      <button className="runtime-account-modal-backdrop" aria-label="Close account window" onClick={onClose}></button>
      <section className="runtime-account-modal" role="dialog" aria-modal="true" aria-label="Account settings">
        ...
        <div className="runtime-account-modal-content">
          ...
          {section === 'profile' ? (
            <div className="runtime-account-modal-section">
              ...
              <div className="runtime-account-profile-fields" aria-label="Profile details">
                <div><span>Name</span><strong>{learnerName}</strong></div>
                <div><span>Email</span><strong>{email}</strong></div>
              </div>
              {isAdmin && (
                <section className="runtime-account-admin-tools" aria-label="Admin tools">
                  <div><h3>Admin tools</h3><p>Operational controls remain separate from the learner navigation.</p></div>
                  <div className="runtime-account-admin-actions">
                    <button onClick={onOpenAdmin}>Open Admin</button>
                    <button onClick={onOpenPlannerAdmin}>Planner assurance</button>
                  </div>
                </section>
              )}
            </div>
