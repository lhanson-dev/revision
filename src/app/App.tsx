...
export function App() {
  ...
  function renderSubjects() {
    return <main className="dashboard screen-dashboard page-screen" aria-labelledby="subjects-page-title"><header className="page-heading"><p className="eyebrow">Your revision programme</p><h1 id="subjects-page-title">Subjects</h1><p>Choose the subject you want to work on. Each subject keeps its own course, paper/component, learning and progress context.</p></header><section className="subject-list" aria-label="Subjects in your revision catalogue">{catalogue.map((subject) => <article className="subject-card subject-card-large" key={subject.id}><div className="subject-mark" aria-hidden="true">{subjectMark(subject)}</div><div className="subject-copy"><span className="tag">Available now</span><h2>{subject.name}</h2><p>{subject.courses.map((course) => `${course.qualificationName} · ${course.examBoardName}`).join(' · ')}</p><p className="muted">{subject.modules.reduce((sum, module) => sum + module.catalogueEntry.topicCount, 0)} topics across {subject.modules.length} {subject.modules.length === 1 ? 'paper/component' : 'papers/components'}.</p></div><button className="primary" onClick={() => navigate(subjectRoute(subject.id))}>Open {subject.name}</button></article>)}</section><p className="quiet-note">Published content packs appear here automatically; the shared learner shell does not contain subject-specific routes or pages.</p></main>
  }
  ...
  function renderModuleScreen(subjectId: string, moduleId: string, requestedSection: PaperSection) {
    ...
    return <main className="dashboard screen-dashboard page-screen paper-screen" aria-labelledby="paper-page-title"><div className="breadcrumbs"><button onClick={() => navigate(subjectsRoute())}>Subjects</button><span>›</span><button onClick={() => navigate(subjectRoute(subjectId))}>{adapter.manifest.subject.name}</button><span>›</span><span>{pLabel}</span></div><header className="page-heading paper-heading"><p className="eyebrow">{cLabel} · specification {adapter.manifest.specificationCode}</p><h1 id="paper-page-title">{adapter.manifest.paper.name}</h1><p>{adapter.manifest.learnerExperience.what_is_this}</p></header>{paperNavigation}{section === 'overview' && <div className="paper-section-content"><section className="paper-recommendation" aria-labelledby="paper-recommendation-title"><div><p className="eyebrow">REV · {pLabel}</p><h2 id="paper-recommendation-title">Your next useful step</h2><p>{recommendation && recommendationTopic ? `${recommendationTopic.shortTitle} · ${activityLabel(recommendation.activity)}. ${recommendation.reason}` : 'Complete a short Practice activity and I’ll use that evidence to guide the next step.'}</p>{recommendation && <p className="muted">{recommendation.limitation}</p>}</div>{sections.includes('practice') && <button className="primary" onClick={() => navigate(moduleRoute(subjectId, moduleId, 'practice'))}>Go to Practice</button>}</section><section className="section-choice-grid" aria-label={`${pLabel} sections`}>{sections.includes('learn') && <button className="section-choice" onClick={() => navigate(moduleRoute(subjectId, moduleId, 'learn'))}><span className="section-icon">L</span><strong>Learn</strong><span>Understand topics, revisit notes and connect ideas.</span></button>}{sections.includes('practice') && <button className="section-choice" onClick={() => navigate(moduleRoute(subjectId, moduleId, 'practice'))}><span className="section-icon">P</span><strong>Practice</strong><span>Flashcards, quick checks, application and exam questions.</span></button>}{sections.includes('exam-prep') && <button className="section-choice" onClick={() => navigate(moduleRoute(subjectId, moduleId, 'exam-prep'))}><span className="section-icon">E</span><strong>Exam Prep</strong><span>Exam technique and realistic timed practice.</span></button>}<button className="section-choice" onClick={() => navigate(moduleRoute(subjectId, moduleId, 'progress'))}><span className="section-icon">✓</span><strong>Progress</strong><span>Coverage, scored evidence and readiness for this paper.</span></button></section><section className="home-section" aria-labelledby="paper-topics-title"><div className="section-heading"><div><p className="eyebrow">Specification areas</p><h2 id="paper-topics-title">{pLabel} topics</h2></div></div><div className="topic-list-grid">{topics.map((topic) => { const hasEvidence = state.evidence.some((item) => item.topicId === topic.id); return <article key={topic.id}><span className={`evidence-dot ${hasEvidence ? 'has-evidence' : ''}`} aria-hidden="true"></span><div><strong>{topic.shortTitle}</strong><p>{hasEvidence ? 'Evidence recorded' : 'No scored evidence yet'}</p></div></article> })}</div></section></div>}{section === 'learn' && <div className="paper-section-content"><FocusedLearningWorkspace adapter={adapter} section="learn" recommendation={recommendation} saving={savingEvidence} saveError={saveError} onRecordEvidence={saveLearningEvidence} />{sections.includes('practice') && <div className="cross-section-next"><div><strong>Ready to test it?</strong><span>Move into Practice without losing this paper context.</span></div><button className="primary" onClick={() => navigate(moduleRoute(subjectId, moduleId, 'practice'))}>Go to Practice</button></div>}</div>}{section === 'practice' && <div className="paper-section-content"><FocusedLearningWorkspace adapter={adapter} section="practice" recommendation={recommendation} saving={savingEvidence} saveError={saveError} onRecordEvidence={saveLearningEvidence} />{sections.includes('exam-prep') && <div className="cross-section-next"><div><strong>Working towards exam conditions?</strong><span>Move into exam technique and realistic timed practice when that is the useful next step.</span></div><button className="primary" onClick={() => navigate(moduleRoute(subjectId, moduleId, 'exam-prep'))}>Go to Exam Prep</button></div>}</div>}{section === 'exam-prep' && <div className="paper-section-content"><FocusedLearningWorkspace adapter={adapter} section="exam-prep" recommendation={null} saving={savingEvidence} saveError={saveError} onRecordEvidence={saveLearningEvidence} />{primaryExam && <section className="exam-simulator-section" aria-label="Full paper simulator"><ExamSimulator exam={primaryExam} moduleId={moduleId} saving={savingEvidence} saveError={saveError} onRecordEvidence={saveLearningEvidence} /></section>}</div>}{section === 'progress' && <div className="paper-section-content"><section className="progress-section-heading"><p className="eyebrow">{pLabel} progress</p><h2>What the evidence says</h2><p>Coverage, understanding evidence and exam readiness are kept distinct. A missing score means more evidence is needed, not that you are failing.</p></section>{moduleProgressSummary(state)}{moduleProgressDetail(state)}<section className="home-section" aria-labelledby="topic-progress-title"><div className="section-heading"><div><p className="eyebrow">Topic evidence</p><h2 id="topic-progress-title">Where you have evidence</h2></div></div><div className="topic-list-grid">{topics.map((topic) => { const count = state.evidence.filter((item) => item.topicId === topic.id).length; return <article key={topic.id}><span className={`evidence-dot ${count > 0 ? 'has-evidence' : ''}`} aria-hidden="true"></span><div><strong>{topic.shortTitle}</strong><p>{count === 0 ? 'No scored evidence yet' : `${count} scored ${count === 1 ? 'activity' : 'activities'}`}</p></div></article> })}</div></section></div>}</main>
  }

  function renderGlobalProgress() {
    return <main className="dashboard screen-dashboard page-screen" aria-labelledby="global-progress-title"><header className="page-heading"><p className="eyebrow">Across your revision</p><h1 id="global-progress-title">Progress</h1><p>Start with the whole picture, then drill into a subject, paper or topic when you need detail.</p></header><section className="home-section" aria-labelledby="global-summary-title"><div className="section-heading"><div><p className="eyebrow">Whole programme</p><h2 id="global-summary-title">Current evidence</h2></div></div>{globalProgressSummary()}</section><div className="subject-list">{catalogue.map((subject) => { const states = moduleStates.filter((state) => state.adapter.manifest.subject.id === subject.id); const topicTotal = states.reduce((sum, state) => sum + state.topicCount, 0); const topicEvidence = states.reduce((sum, state) => sum + state.evidencedTopics, 0); const readinessScores = states.filter((state) => state.readiness.score !== null); return <section className="global-progress-card" aria-labelledby={`progress-${subject.id}`} key={subject.id}><div className="subject-mark" aria-hidden="true">{subjectMark(subject)}</div><div className="global-progress-copy"><p className="eyebrow">{subject.name}</p><h2 id={`progress-${subject.id}`}>{topicEvidence} of {topicTotal} topics have evidence</h2><p>{states.length} {states.length === 1 ? 'paper/component' : 'papers/components'} in the current catalogue.</p><p className="muted">{readinessScores.length === 0 ? 'Readiness is still building enough evidence.' : `${readinessScores.length} ${readinessScores.length === 1 ? 'paper has' : 'papers have'} a supported readiness score.`}</p></div><button className="primary" onClick={() => navigate(subjectRoute(subject.id))}>Open {subject.name}</button></section> })}</div><section className="home-section" aria-labelledby="global-recent-title"><div className="section-heading"><div><p className="eyebrow">Recent evidence</p><h2 id="global-recent-title">Latest activity</h2></div></div>{globalActivity.length === 0 ? <p className="muted">No scored activity yet.</p> : <ul className="activity-list">{globalActivity.slice(0, 5).map((item) => { const adapter = getContentAdapter(item.moduleId); return <li key={item.id}><strong>{adapter ? `${adapter.manifest.subject.name} · ${item.label}` : item.label}</strong><span>{item.detail}</span><time dateTime={item.occurredAt}>{new Date(item.occurredAt).toLocaleDateString()}</time></li> })}</ul>}</section></main>
  }

  function renderRev() {
    return <main className="dashboard screen-dashboard page-screen rev-page" aria-labelledby="rev-page-title"><header className="page-heading"><p className="eyebrow">Your AI guide</p><h1 id="rev-page-title">REV</h1><p>REV keeps the wider revision picture in view, then narrows into the subject, paper or activity you are working on.</p></header><section className={`rev-hero rev-page-hero ${revSuggested ? 'rev-awake' : ''}`} aria-labelledby="rev-conversation-title"><div className="rev-copy"><div className="rev-pill"><span aria-hidden="true">✦</span> REV</div><h2 id="rev-conversation-title">What do you want help with?</h2><p className={`rev-message ${displayedRevTyping ? 'typing' : ''}`} aria-live="polite">{displayedRevMessage}</p><div className="rev-actions"><button className="rev-primary" onClick={suggestNextStep}>What should I revise?</button><button className="rev-secondary" onClick={() => navigate(subjectsRoute())}>Show my subjects</button>{recommendedSubject && <button className="rev-secondary" onClick={() => navigate(subjectRoute(recommendedSubject.id))}>Open {recommendedSubject.name}</button>}</div></div><div className="rev-orb-wrap" aria-hidden="true"><div className="rev-orb"><span className="orb-ring ring-one"></span><span className="orb-ring ring-two"></span><span className="orbit-dot"></span><span className="orb-core"></span></div></div></section><p className="quiet-note">The current REV recommendation is deterministic and compares the evidence state of every published paper/component. The conversational tutor layer can build on the same catalogue context without changing this hierarchy.</p></main>
  }
