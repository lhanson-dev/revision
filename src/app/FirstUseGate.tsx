...
function resolveStarterTarget(
  ...
  const freshQuestion = adapter.listQuestions(topicId).find((question) => !startingQuestionIds.has(question.id))
  if (freshQuestion || adapter.listQuestions(topicId).length > 0) return { topicId, activity: 'quick-check' }
  return null
}
...
function feedbackCopy(evidence: LearningEvidence | null) {
  ...
  return {
    title: 'Your first useful revision is complete.',
    body: 'Revision now has real learning evidence to use when deciding what should come next.',
  }
}
...
function courseIdentity(item: CatalogueCourseItem) {
  ...
  return [subject.name, course.qualificationName, boardNeeded ? course.examBoardName : null, `Specification ${course.specificationCode}`].filter(Boolean).join(' · ')
}
...
export function FirstUseGate({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    let active = true
    if (!user || !adapter || accountState?.onboardingCompletedAt) {
      setStartingEvidence([])
      setNormalEvidence([])
      return () => { active = false }
    }
    ...
    const startingStore = createSupabaseStartingCheckEvidenceStore(supabase)
    const evidenceStore = createSupabaseEvidenceStore(supabase)
    Promise.all([
      loadStartingCheckEvidence(startingStore, user.id, adapter.manifest.id),
      loadLearningEvidence(evidenceStore, user.id, adapter.manifest.id),
    ])
      ...
      .catch((reason: unknown) => {
        if (active) setError(reason instanceof Error ? reason.message : 'Could not load your starting point.')
      })
  ...
  const resolvedCourseItem = boardCourses.length === 1
    ? boardCourses[0]
    : boardCourses.find((item) => item.course.id === courseId) ?? null
  ...
  if (stage === 'starting_check' && !evidenceLoading && selectedStartingQuestions.length > 0 && !currentStartingQuestion) stage = 'recommendation'
  if (stage === 'starting_check' && !evidenceLoading && selectedStartingQuestions.length === 0) stage = 'recommendation'
  ...
  }, [currentStartingQuestion?.id])

  useEffect(() => {
    setShowFlashcardAnswer(false)
    setActivityOption(null)
  }, [accountState?.starterActivity, accountState?.starterTopicId])
  ...
  async function chooseStudent() {
    ...
    try {
      ...
      const message = reason instanceof Error ? reason.message : 'Could not save your Student experience.'
      setError(message)
  ...
  async function addFirstCourse() {
    ...
    try {
      ...
      const nextState = await setFirstUseStage(supabase, user.id, 'course_ready')
      setAccountState(nextState)
    } catch (reason: unknown) {
      const message = reason instanceof Error ? reason.message : 'Could not add that course.'
      setError(message)
  ...
  async function beginStartingCheck() {
    ...
    try {
      ...
      if (selectedStartingQuestions.length === 0) {
        const nextState = await setFirstUseStage(supabase, user.id, 'recommendation')
        setAccountState(nextState)
        ...
        const nextState = await setFirstUseStage(supabase, user.id, 'starting_check')
        setAccountState(nextState)
  ...
  async function skipStartingCheck() {
    ...
    try {
      const nextState = await setFirstUseStage(supabase, user.id, 'recommendation')
      setAccountState(nextState)
      const eventType = startingEvidence.length > 0 ? 'starting_check_partial' : 'starting_check_skipped'
      void recordFirstUseEventBestEffort(supabase, user.id, eventType, activeCourseItem.course.id, { answered_count: startingEvidence.length })
    } catch (reason: unknown) {
  ...
  async function answerStartingQuestion() {
    ...
    try {
      ...
      const message = reason instanceof Error ? reason.message : 'Could not save that answer.'
      setError(message)
  ...
  async function startRevision() {
    ...
    setError('')
    try {
      const nextState = await startFirstUseActivity(
        supabase,
        user.id,
        chosenStarterTarget.topicId,
        chosenStarterTarget.activity,
      )
      setAccountState(nextState)
  ...
  async function finishActivity(evidence: LearningEvidence) {
    ...
    try {
      ...
      const nextState = await setFirstUseStage(supabase, user.id, 'feedback')
      setAccountState(nextState)
  ...
  if (loading) return <main className="first-use-shell" data-theme={theme}><BrandAsset asset="wordmark" className="first-use-brand" alt="Revision" /><div className="first-use-loading">Loading your Revision setup…</div></main>

  if (error && !user) {
    return <main className="first-use-shell" data-theme={theme}><BrandAsset asset="wordmark" className="first-use-brand" alt="Revision" /><section className="first-use-card"><Status tone="error">{error}</Status><Button onClick={() => setRefreshKey((value) => value + 1)}>Try again</Button></section></main>
  }
      ...
      <main className="first-use-shell" data-theme={theme}>
        ...
        <section className="first-use-card first-use-choice-screen" aria-labelledby="experience-heading">
          ...
          <div className="first-use-experience-list" role="group" aria-label="Revision experience type">
            <ExperienceChoice title="Student" description="Plan, learn, practise and get guidance from REV." available={!busy} onSelect={() => void chooseStudent()} />
          ...
          <div className="first-use-course-fields">
            {qualificationOptions.length > 1 ? (
              <SelectField label="Qualification" value={qualification} onChange={(event) => { setQualification(event.target.value); setSubjectId(''); setExamBoard(''); setCourseId('') }}>
                ...
                {qualificationOptions.map((value) => <option key={value} value={value}>{value}</option>)}
              </SelectField>
            ) : effectiveQualification && <div className="first-use-auto-choice"><span>Qualification</span><strong>{effectiveQualification}</strong></div>}
            ...
            ) : subjectOptions[0] && <div className="first-use-auto-choice"><span>Subject</span><strong>{subjectOptions[0][1]}</strong></div>)}
            ...
            {effectiveSubjectId && (examBoardOptions.length > 1 ? (
              <SelectField label="Exam board" value={examBoard} onChange={(event) => { setExamBoard(event.target.value); setCourseId('') }}>
                ...
                {examBoardOptions.map((value) => <option key={value} value={value}>{value}</option>)}
              </SelectField>
            ) : effectiveExamBoard && <div className="first-use-auto-choice"><span>Exam board</span><strong>{effectiveExamBoard}</strong></div>)}
            ...
            {effectiveExamBoard && boardCourses.length > 1 && (
              <SelectField label="Course" value={courseId} onChange={(event) => setCourseId(event.target.value)}>
                ...
                {boardCourses.map((item) => <option key={item.course.id} value={item.course.id}>{item.label} · {item.course.specificationCode}</option>)}
              </SelectField>
            )}
          </div>
  ...
  if (!activeCourseItem || !adapter) {
    return (
      <main className="first-use-shell" data-theme={theme}>
        <BrandAsset asset="wordmark" className="first-use-brand" alt="Revision" />
        <section className="first-use-card"><Status tone="error">Your saved course is not available in the current published catalogue. Revision will not silently substitute a different course.</Status><Button onClick={() => setRefreshKey((value) => value + 1)}>Try again</Button></section>
      </main>
    )
  }
      ...
      <main className="first-use-shell" data-theme={theme}>
        ...
        <section className="first-use-card first-use-ready-screen" aria-labelledby="course-ready-heading">
          ...
          <div className="first-use-ready-action">
            ...
            <div className="first-use-ready-actions"><Button size="large" disabled={busy} onClick={() => void beginStartingCheck()}>Find my starting point</Button><Button variant="tertiary" disabled={busy} onClick={() => void skipStartingCheck()}>Skip for now</Button></div>
          </div>
        </section>
      </main>
    )
  ...
  if (stage === 'starting_check') {
    if (evidenceLoading) return <main className="first-use-shell" data-theme={theme}><BrandAsset asset="wordmark" className="first-use-brand" alt="Revision" /><div className="first-use-loading">Preparing your short starting check…</div></main>
    if (!currentStartingQuestion) {
      return <main className="first-use-shell" data-theme={theme}><BrandAsset asset="wordmark" className="first-use-brand" alt="Revision" /><div className="first-use-loading">Working out a useful place to begin…</div></main>
    }
        ...
        <section className="first-use-card first-use-check-screen" aria-labelledby="starting-check-heading">
          ...
          <fieldset className="first-use-options" disabled={busy}>
            ...
            {currentStartingQuestion.options.map((option, index) => (
              <label key={option} className={startingOption === index ? 'is-selected' : ''}>
                <input type="radio" name="starting-check-answer" checked={startingOption === index} onChange={() => setStartingOption(index)} />
                <span>{option}</span>
              </label>
            ))}
          </fieldset>
  ...
  if (stage === 'recommendation') {
    if (evidenceLoading) return <main className="first-use-shell" data-theme={theme}><BrandAsset asset="wordmark" className="first-use-brand" alt="Revision" /><div className="first-use-loading">Working out a useful place to begin…</div></main>
    if (!baseStarterTarget || !chosenStarterTarget) {
      return (
        <main className="first-use-shell" data-theme={theme}>
          <BrandAsset asset="wordmark" className="first-use-brand" alt="Revision" />
          <section className="first-use-card"><Status tone="error">This course does not currently contain an assured starter activity Revision can route you into. Your course has been kept; no weakness or result has been invented.</Status><Button onClick={() => setRefreshKey((value) => value + 1)}>Try again</Button></section>
        </main>
      )
    }
    ...
    return (
      <main className="first-use-shell" data-theme={theme}>
        ...
        <section className="first-use-card first-use-recommendation-screen" aria-labelledby="recommendation-heading">
          ...
          {!showAlternatives ? (
            <Button variant="tertiary" onClick={() => setShowAlternatives(true)}>Choose something else</Button>
          ) : (
  ...
  const persistedStarter = adapter ? resolveStarterTarget(adapter, activityTopicId, startingEvidence) : null
  ...
  if (stage === 'activity') {
    if (!activityTopicId || !activityMode || !activityTopic) {
      return <main className="first-use-shell" data-theme={theme}><BrandAsset asset="wordmark" className="first-use-brand" alt="Revision" /><section className="first-use-card"><Status tone="error">Revision could not restore the exact first activity. Your saved course and starting evidence are still safe.</Status><Button onClick={() => setRefreshKey((value) => value + 1)}>Try again</Button></section></main>
    }
  ...
  return <main className="first-use-shell" data-theme={theme}><BrandAsset asset="wordmark" className="first-use-brand" alt="Revision" /><div className="first-use-loading">Preparing Revision…</div></main>
}
