...
const liveEnabled = env.CONTENT_FACTORY_FOUNDATION_INTERNAL_LEARNING_REPEAT_REASSURANCE_PROOF === '1'
const evidenceDirectory = '.artifacts/content-factory-foundation-internal-learning-repeat-reassurance-proof'
...
describe('Foundation-native cumulative-context Learn/Practice re-assurance proof', () => {
  const liveIt = liveEnabled ? it : it.skip

  liveIt('deterministically validates and freshly reviews the latest corrected bundle without any prior reviewer reuse', async () => {
    ...
    try {
      ...
      const workers: FoundationInternalLearningAssuranceWorkers = {
        async independentReview(input) {
          const execution = await client.run({
            workerId: 'content-factory.foundation-internal-learning-repeat-remediation-reassurance-review',
            contractVersion: '1',
      ...
      const priorReviewerContextIds = remediationProof.remediationRecord.priorReviewerContextIds
      expect(new Set(priorReviewerContextIds).size).toBe(priorReviewerContextIds.length)
      ...
      const reassuranceCycle = remediationProof.remediationCycle ?? 1
      const evidenceBase = {
        schemaVersion: 1,
      ...
      if (result.status !== 'pass' || !result.assuredAssets) {
        ...
        await addIssueComment(repo, token, [
          'Foundation-native latest corrected Learn/Practice re-assurance did not pass.',
          '',
          '- Course: **AQA A-level Business 7132 — 2027 cohort**',
          `- Foundation fingerprint: \`${foundationFingerprint}\``,
          `- Re-assurance cycle: **${reassuranceCycle}**`,
          ...
          `- Prior reviewer contexts excluded: **${priorReviewerContextIds.length}**`,
        ...
        throw new Error(`Foundation-native latest corrected Learn/Practice re-assurance returned ${result.status}`)
      }
      ...
      await addIssueComment(repo, token, [
        'Foundation-native latest corrected Learn/Practice re-assurance passed.',
        '',
        '- Course: **AQA A-level Business 7132 — 2027 cohort**',
        `- Foundation fingerprint: \`${foundationFingerprint}\``,
        `- Re-assurance cycle: **${reassuranceCycle}**`,
