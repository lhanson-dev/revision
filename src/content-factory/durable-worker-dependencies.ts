...
// The first Q7 soak then exposed a generic Assessment Item omission class before
// the existing bounded repair boundary. output-integrity-v3 moved that parseable
// omission class inside one complete-diagnostic repair. The second Q7 soak exposed
// a duplicated coverageEvidence.requirementId cross-reference that could disagree
// with the owning subquestion requirementIds even after repair. output-integrity-v4
// replaces that provider-authored duplicate with a bounded requirementPosition
// locator and resolves the final durable requirementId deterministically.
