# Information Architecture

**Status:** Draft authority candidate — v0.1  
**Purpose:** Define the initial top-level structure of the Revision student experience.

## Principle

The information architecture should make the product feel clear, calm and purposeful.

The student should not have to understand the internal complexity of Revision in order to use it. The top-level structure should reflect the jobs they are trying to do: understand what matters now, move between subjects, practise, prepare for exams, see progress and get support.

## Top-level navigation

### Home

Home is the default landing area and should answer the most important question first:

> What should I do now?

The primary call to action should be the recommended next action based on the student's wider revision picture.

Home should also provide a small number of supporting signals and signposts, such as:

- current progress or readiness;
- recent activity;
- relevant warnings or encouragement;
- routes into Subjects, Practice, Exam Prep, Progress and Tutor.

Home should remain focused. It should not become a busy activity feed or dashboard full of competing widgets.

### Subjects

Subjects is the student's structured view of the subjects they have added.

Students with more than one subject must be able to switch between them easily.

Within a subject, the student should be able to browse the underlying course/specification structure and see topic-by-topic coverage and progress rather than having the syllabus hidden behind recommendations.

Subject areas may surface relevant learning resources, practice options, weaknesses and recommended next actions.

### Practice

Practice groups active revision and assessment activities, including:

- flashcards;
- quizzes;
- topic tests;
- exam questions; and
- other active-recall formats.

Practice should feed evidence back into the wider student model and provide useful explanatory feedback, not just scores.

### Exam Prep

Exam Prep brings together activity specifically intended to prepare the student for exam conditions, including:

- timed practice;
- full papers;
- targeted exam-question work; and
- the Exam Simulator.

As an exam approaches, Revision may increasingly signpost this area as part of the student's recommended plan.

### Progress

Progress should help the student understand the bigger picture across subjects and topics, including:

- specification coverage;
- mastery or understanding;
- weaknesses;
- recent progress; and
- exam readiness where supported by sufficient evidence.

Progress should provide confidence when the student is on track and constructive direction when they are not.

### Tutor

Tutor is the dedicated space for the ongoing AI tutor relationship.

The tutor should be able to use the student's subjects, progress, activity and wider context to provide personalised guidance, explanations, encouragement and recommendations.

Tutor should also be contextually accessible across the product rather than confined to a single page. A student should be able to ask for help while viewing a topic, completing practice, reviewing feedback or preparing for an exam.

## Navigation model

The initial student navigation is therefore:

1. Home
2. Subjects
3. Practice
4. Exam Prep
5. Progress
6. Tutor

This is an initial authority baseline, not a permanent commitment to exact labels or navigation mechanics. Usability research and product evidence may justify later changes while preserving the underlying jobs and product principles.

## Supporting experiences

Account, settings, subject management and similar utility functions should remain secondary to the main revision journey.

Parent and teacher experiences should be treated as separate role-specific journeys when developed, rather than being inserted into the student's everyday navigation.

## Design guardrails

The information architecture should:

- keep the recommended next action prominent;
- support fast switching between multiple subjects;
- keep the specification transparent and browsable;
- make the AI tutor available in context;
- avoid excessive dashboard density;
- preserve student freedom to navigate and choose their own activity; and
- make progress and exam preparation easy to find without overwhelming the primary revision flow.
