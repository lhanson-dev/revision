...
### Full-shell visual reproduction rule

When a visual artefact shows the full shell of an existing Revision surface, the inherited shell must come from one of these evidence-backed methods:

1. an actual captured screenshot of the current approved runtime or retained visual-regression baseline; or
2. a code-rendered reproduction that uses the current approved runtime structure, design tokens and relevant shell/navigation CSS from `main`.

A generative image tool must **not** redraw or reinterpret the existing learner shell, desktop rail, responsive drawer, course chrome, REV identity or Light/Dark theme treatment. Generative work may be used only inside the bounded area deliberately being redesigned, while the inherited shell remains screenshot-backed or code-rendered.

If neither evidence-backed method is available, the artefact must remain bounded to the changed area and must not show an invented surrounding shell.

This rule exists because visual similarity is not sufficient evidence of fidelity. The Founder should never have to infer whether a changed sidebar colour, navigation pattern, header, icon or account treatment is a deliberate proposal or an artefact of the visualisation method.
...
- If the full shell is visible, is it screenshot-backed or code-rendered from current approved implementation evidence rather than regenerated by an image model?
